...
interface FormObjectGroup<T extends FormFields>
  extends FormFieldBase<FormFieldsValue<T>> {
  type: "object";
  fields: T;
}
...
function getValuesFromFields<T extends FormFields = FormFields>(
  fields: T
): FormFieldsValue<T> {
...
function createFormObjectGroup<T extends FormFields>(
  fields: T,
  config: FormFieldConfig<T> = {}
): FormObjectGroup<T> {
...
export function CaseConfig(props: {
  defaultValue?: Config;
  ...
  const form = useMemo(() => {
    ...
    const group = createFormObjectGroup({
      runningMode: createFormField(
        defaultValue?.duration !== Infinity
          ? RunningMode.ONCE
      ...
      <$Header>
        <ButtonGroup css={$mgr8}>
          <Button
            ...
            icon={
              runningMode.value === RunningMode.ONCE ? "small-tick" : undefined
            }
          >
            单次测速
          </Button>
          <Button
            intent={
              runningMode.value === RunningMode.CONTINUE ? "success" : "none"
            }
            onClick={() => runningMode.onChange(RunningMode.CONTINUE)}
            icon={
              runningMode.value === RunningMode.CONTINUE
                ? "small-tick"
                : undefined
            }
          >
      ...
      <Collapse isOpen={isAdvancedConfig}>
        ...
        >
          <RadioGroup
            ...
            onChange={(e) =>
              speedRange.onChange(e.currentTarget.value as SpeedMode)
            }
            inline={true}
          >
        ...
        <FormGroup
          label="Parallal"
          helperText="并行数量，推荐 3 个，如果想要测试单线程，可以调整为 1"
        >
          <Slider
            min={1}
            max={16}
