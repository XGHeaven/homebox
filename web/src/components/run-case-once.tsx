...
export function RunCaseOnce() {
  ...
  const start = async () => {
    ...
    await new Promise((resolve, reject) => {
      interval(300).pipe(take(10), mergeMap(() => from(ping()))).subscribe(v => {
        pushTTL(v)
      }, reject, resolve)
    })
    ...
    <div>
      <$Header>
        <$HeaderCase>
          ...
          <$CaseContent>
            {step >= RunningStep.PING ? `${ttl.toFixed(2)} ms` : "--"}
          </$CaseContent>
        </$HeaderCase>
        ...
        <$HeaderCase>
          ...
          <$CaseContent>
            {step >= RunningStep.DOWNLOAD ? rateFormatters[unit](dlRate) : "--"}
          </$CaseContent>
        </$HeaderCase>
        ...
        <$HeaderCase>
          ...
          <$CaseContent>
            {step >= RunningStep.UPLOAD ? rateFormatters[unit](ulRate) : "--"}
          </$CaseContent>
        </$HeaderCase>
      </$Header>
      <SpeedIndicator
        speed={
          step === RunningStep.DOWNLOAD
            ? dlRate
            : step === RunningStep.UPLOAD
            ? ulRate
            : undefined
        }
      ...
      <div css={css`${$textCenter}${$mgt[4]}`}>
        <Button
          onClick={start}
          disabled={step !== RunningStep.NONE && step !== RunningStep.DONE}
        >
          Start
        </Button>
      </div>
